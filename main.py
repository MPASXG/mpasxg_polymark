...
def index():
    ...
    <html>
    <head>
        <title>Yield Hunter Nebula 75k - Unlocked</title>
    ...
    <body class="bg-[#02040a] text-slate-300 p-4 md:p-10 font-sans">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-10 bg-slate-900/20 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl backdrop-blur-3xl">
                <div class="text-center md:text-left">
                    ...
                    <p class="text-slate-500 text-[10px] mt-2 uppercase tracking-[0.4em] font-black opacity-70 italic">
                        Unlocked Mode | No Result Limit | 75,000 Markets
                    </p>
                </div>
                <div class="mt-6 md:mt-0 flex flex-col items-end">
                    <div id="progress" class="text-blue-400 font-mono text-xs bg-blue-500/10 px-8 py-4 rounded-3xl border border-blue-500/20 shadow-lg">
                        Prêt pour le grand scan...
                    </div>
                    <div id="match-counter" class="mt-2 text-[10px] font-bold text-green-500 uppercase tracking-widest">0 pépites trouvées</div>
                </div>
            </div>
            
            <div class="bg-slate-900/30 rounded-[2.5rem] border border-white/5 shadow-2xl overflow-hidden">
                <div class="overflow-x-auto max-h-[800px] overflow-y-auto">
                    <table class="w-full text-left border-collapse text-[11px] md:text-xs">
                        <thead class="sticky top-0 bg-[#0a0f1d] text-slate-500 uppercase font-black tracking-widest text-[9px] z-10">
                            <tr>
        ...
        <script>
            ...
            async function runNebulaScan() {
                const totalBlocks = 75; 
                
                for (let i = 0; i < totalBlocks; i++) {
                    const offset = i * 1000;
                    progress.innerText = `SCAN EN COURS : ${i+1}/${totalBlocks}`;
                    ...
                    try {
                        ...
                        if (chunk && chunk.length > 0) {
                            // On ajoute tout sans aucune limite
            ...
            function renderTable(data) {
                ...
                for (const m of data) {
                    ...
                    <tr class="hover:bg-blue-500/[0.04] transition-colors group">
                        ...
                        <td class="p-6 text-right font-mono text-blue-500 text-sm font-black italic group-hover:scale-105 transition-transform origin-right leading-none">
                            ${m.yield.toFixed(2)}
                        </td>
                    </tr>`;
                }
...
def scan_chunk():
    ...
    try:
        ...
        # --- MODIFICATION ICI : On retourne TOUT, pas de limite [:150] ---
